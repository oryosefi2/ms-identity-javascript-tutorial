...
Function Cleanup
{
    ...
    {
	    Write-Host "Unable to remove the application 'ms-identity-javascript-c1s1' . Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (ms-identity-javascript-c1s1) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'ms-identity-javascript-c1s1'"
    ...
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'ms-identity-javascript-c1s1'" | ForEach-Object {Remove-MgServicePrincipal -ApplicationId $_.Id -Confirm:$false}
    }
    catch
    {
	    Write-Host "Unable to remove ServicePrincipal 'ms-identity-javascript-c1s1' . Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
}
